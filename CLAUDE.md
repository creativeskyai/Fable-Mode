...
2. **Config** — `.claude/fable/CONFIG.md`. User-owned defaults (subagent model/effort, fleet, votes). Skills read it at invocation and pass its yaml keys as `config` in workflow args; workflows apply it via the shared config block. Once installed it belongs to the user: `--update` never overwrites it (both installers special-case `fable/CONFIG.md`, CI asserts it), and `/fable-config` carries a template to recreate it. Same ownership model for `.claude/settings.json`: the installers materialize it from `.claude/fable/settings.template.json` only when missing (pre-approving Workflow/Agent, disclosed in SECURITY.md), never modify an existing one, and never ship a root-level `settings.json` (basename-skipped in the copy loop, gitignored in this repo — CI's file-count check expects tracked+1 because of the materialization).
